...
1. Create a client specific configuration:
        ...
        $ echo "ifconfig-push 192.168.254.1 192.168.254.2" | docker run --volumes-from $OVPN_DATA -i --rm farfeduc/openvpn tee /etc/openvpn/ccd/CERT_COMMON_NAME
...
Login to the data volume with a `bash` container, note only changes in /etc/openvpn will persist:
    ...
    docker run --volumes-from $OVPN_DATA -it --rm farfeduc/openvpn bash -l
...
If you're running an old configuration and need to upgrade it to pull in the ccd directory run the following:
    ...
    docker run  --volumes-from $OVPN_DATA --rm farfeduc/openvpn ovpn_genconfig
